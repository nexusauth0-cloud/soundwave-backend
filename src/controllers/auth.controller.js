  ...
  try {
    ...
    if (!rows.length) {
      ...
      const created = await db.query(
        `INSERT INTO users (email, username, display_name, is_verified) VALUES ($1,$2,$3,TRUE) RETURNING id`,
...
async function me(req, res) {
  try {
    const { rows } = await db.query(
      `SELECT id, email, username, display_name, avatar_url, role, plan, is_verified, created_at FROM users WHERE id=$1`,
      [req.user.id]
    );
